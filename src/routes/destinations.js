...
router.get('/', requireAuth, async (req, res) => {
  try {
    const { rows } = await query(`
      ...
      FROM public.destinations ORDER BY name ASC
    `);
    res.json(rows);
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const { rows } = await query(`
      INSERT INTO public.destinations (id, name, api_url, color, field_schema, driver_slug, auth, enabled)
      VALUES (COALESCE($1, gen_random_uuid()::text), $2, $3, $4, $5, $6, $7, true)
      RETURNING *
    `, [
      id || null,
      name.trim(),
...
router.patch('/:id/toggle', requireAuth, async (req, res) => {
  try {
    const { rows } = await query(`
      UPDATE public.destinations SET enabled = NOT enabled, updated_at = now()
...
router.delete('/:id', requireAuth, async (req, res) => {
  try {
    await query('DELETE FROM public.unit_destinations WHERE destination_id = $1', [req.params.id]);
    const { rows } = await query(
      'DELETE FROM public.destinations WHERE id = $1 RETURNING id', [req.params.id]
    );
    if (!rows.length) return res.status(404).json({ error: 'Destino no encontrado.' });
