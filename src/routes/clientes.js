...
router.post('/login', async (req, res) => {
  ...
  const { rows } = await query(
    `SELECT id, nombre, rut, token, enabled, password_hash
     ...
     WHERE REPLACE(REPLACE(rut, '.', ''), '-', '') = REPLACE(REPLACE($1, '.', ''), '-', '')`,
    [rut.trim()]
  );
  ...
  if (!cliente.password_hash)
    return res.status(401).json({ error: 'Acceso no configurado. Contacta al administrador.' });
...
router.get('/me/units', async (req, res) => {
  ...
  // Devolver también el token de cliente para las llamadas al GPS proxy
...
/* ── POST /clientes/:id/set-password (admin asigna contraseña) ── */
